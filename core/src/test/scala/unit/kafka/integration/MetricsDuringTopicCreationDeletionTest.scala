...
class MetricsDuringTopicCreationDeletionTest extends KafkaServerTestHarness with Logging {
  ...
  override def setUp {
    ...
    for (m <- testedMetrics) {
        val metricName = SharedMetricRegistries.getOrCreate("default").getMetrics.asScala.keys.find(extractName(_).endsWith(m))
        metricName.foreach(SharedMetricRegistries.getOrCreate("default").remove)
    }
  ...
  private def getGauge(metricName: String) = {
    SharedMetricRegistries.getOrCreate("default").getMetrics.asScala
           .filterKeys(k => extractName(k).endsWith(metricName))
           .headOption
